 ...
 * 
 * PURPOSE: MONTE CARLO BY READING FILES CONTAINING harmonic means averages in the forms of vectors. Based off of
 * processFilesCritical.cpp which was created like march 24, 2023 or sth.. This one might be easily generalizable
 * and I feel this .cpp script has little difference from previous versions.
 * 
 * I'll see if I can make it a bit more general here.
 * 
 * 
 *  
 * MARCH 24 NOTES:
...
{
    ifstream File;
    vector<int>reportFileVector(7);//<--for crit densit it is in summVEC, for hyperunif in Summ hyperunif 
    //or sth like that
    long double number;
    

    const int numb_data_pts=27; //<--for crit density it is #of hmeans, for hyperunif it's #of lengths
    ...
    vector<vector<double>>ARRAY_TO_AVGE(NUMBERofTASKSinJOBARRAY, vector<double>(numb_data_pts));
   ...
   for(int taskNum=0; taskNum<NUMBERofTASKSinJOBARRAY; taskNum++)
   {
       ...
       for(int elem=0; elem< pow2; elem++)
        {   
            File>>number;
            ARRAY_TO_AVGE[taskNum][elem]=number;
            }
        
        File.close();    
            
        cout<<"We have so far at task number "<< taskNum<< ", ARRAY_TO_AVGE["<<taskNum<<"]= \n"<<endl;  
        DisplayVector(ARRAY_TO_AVGE[taskNum]);
        cout<<"\n";
    ...
    *==================================================================================== */
    ...
    vector<double> averageAcrossFiles(number_data_pts);
    
    //Says hMeanNumber, but not really hMeans at this point anymore, it's historical convention

    for(int hMeanNumber=0; hMeanNumber<number_data_pts; hMeanNumber++)
    {
        runningMean=0.0;
        for(int taskNum=0; taskNum<NUMBERofTASKSinJOBARRAY; taskNum++)
        {
            number=ARRAY_TO_AVGE[taskNum][hMeanNumber];
            runningMean+=number;
            }
    ...
    *==================================================================================== */
    ...
    string fileName="AvgeFile"+parameters+'_'+timestr; //<--eliminated date in FileName, could cause problms
